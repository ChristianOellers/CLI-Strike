...
function onGameLoopComplete(round) {
  if (!round.attack) {
    // No enemy encountered — skip outcome, loop back
...
<template>
  ...
  <main class="main">
    <PhaseSelection
      v-if="state.phase === 'selection'"
      @complete="onSelectionComplete"
    />
    <PhaseGameLoop
      v-else-if="state.phase === 'game_loop'"
      :player="state.player"
      @complete="onGameLoopComplete"
    />
    <PhaseOutcome
      v-else-if="state.phase === 'outcome'"
