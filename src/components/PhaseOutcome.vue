...
import { WIN_REACTIONS, GRAFFITI_OPTIONS, LOSE_REACTIONS } from '../game-data.js'
...
<template>
  <div class="phase-card" :class="isWin ? 'phase-card--win' : 'phase-card--lose'">
    <div>
      <h2 class="phase-card__title">{{ isWin ? 'You win!' : 'You lose!' }}</h2>
    ...
    <ChoiceList
      ...
      :options="GRAFFITI_OPTIONS"
      v-model="graffiti"
    />

    <button class="btn btn--primary" :disabled="!canSubmit" @click="emit('next')">
      Next round →
    </button>
  </div>
</template>
